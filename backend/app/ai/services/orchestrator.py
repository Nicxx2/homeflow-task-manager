...
class AIOrchestratorService:
    ...
    def parse_assistant_intent(self, *, message: str, visible_members: list[str], today: str) -> dict | None:
        ...
        prompt = (
            ...
            "date YYYY-MM-DD|null, date_field due|assignment|completed|either, "
            "assignee me|unassigned|one visible member name|null.\n"
            "Use date_field completed for completed/done/finished on a date. Use assignment for who has or had "
            "tasks on a date. Use due only when due/expected is explicit or the request is unassigned due work.\n"
            "Do not invent member names or dates. If the user asks to create, delete, edit, move, reschedule, "
            "or complete tasks, use unsupported_action because app-owned confirmation flows must handle writes.\n"
